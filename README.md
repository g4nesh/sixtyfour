...
`POST /api/research` returns `application/x-ndjson`. Every stream ends in one terminal event, including refusals, cancellation, configuration errors, partial results, and failures. Optional `requestedCategories` values must use the exact schema names `identity`, `employment`, `education`, `publication`, or `online_presence`; an empty list, natural-language alias, or unknown value is rejected with HTTP 400 instead of being silently dropped.
...
Each frontier edge has a strictly positive immutable cost. The path cost is the parent path cost plus that edge cost; selection sorts by path cost, source tier, depth, insertion ordinal, then ID. Dominated duplicate pivots are pruned, and a higher website tier cannot execute while a legal lower-tier entry remains. If a later broad search exposes one exact classified lead or an evidence-grounded Keybase handle in an earlier tier, only that bounded dependency may run off-cursor; arbitrary earlier-tier breadth stays closed and the monotonic breadth cursor does not regress. Search utility combines relevance, novelty, expected information gain, source prior, transport cost, policy risk, repetition, and depth penalty. These values choose what to investigate next; they never increase evidence or finding confidence.

One seeded proposal slot per batch may use a finite neighboring policy. The acceptance decision is a deterministic SHA-256 draw under a cooled Metropolis-Hastings ratio with forward/reverse neighbor correction. Opaque lead capabilities and exact-handle Keybase dependencies are never mutation parents. Other accepted mutations must still pass every source, safety, candidate, budget, and evidence gate, and executed mutation actions may not exceed 20% of completed tool actions. Rejected, unselected, and exhausted mutation nodes stay in the report graph.
...
For a public-professional name, role, or organization, `compileOsintQueries` creates at most 16 auditable query instructions. Human-name normalization preserves Unicode scripts while generic tool identifiers retain a separate ASCII-only grammar. The first query is an untouched quoted exact-match baseline. Later variants may add a separately labeled noise-exclusion refinement; exact organization, role, and coarse-location context; mechanically derived initials or punctuation folding; closed `site:` scopes for GitHub, LinkedIn, ORCID, Google Scholar, OpenReview, Semantic Scholar, OpenAlex, and official App Store listings; up to two explicitly admitted academic domains; and a bounded `filetype:pdf`/`intitle:` document pivot. A selected frontier entry—not model prose—owns the query that actually reaches the search transport, and every returned URL remains discovery-only until an exact hardened fetch succeeds.

Each admitted candidate-bound discovery lead opens one opaque, lane-checked `fetch_public_source` frontier entry. The kernel binds it to the exact observed HTTPS URL and deduplicates by candidate plus canonical URL. If separate searches fetch the same canonical page for the same normalized page subject, Atlas reuses that page-subject branch only when one prior candidate already has both supporting direct-fetch evidence and a strong evidence-grounded profile URL for that exact page. Ambiguous, ungrounded, and cross-URL reuse attempts fail closed; this exception is page deduplication, not a person merge.
