...
test("provider annotations authorize only their opaque candidate-scoped lead, while content URLs and query variants do not", async () => {
  ...
  assert.equal(searchResult.evidence[0].title, "Public source at profile.example",
  ...
  assert.equal(contentOnly.diagnostics.some((item) => item.code === "github_exact_name_not_observed"), false);
});
...
test("successful authorized HTML metadata survives extraction provider 503 without evidence authority", async () => {
  ...
  const dependencies = createLiveDependencies(engine.snapshot().input, {
    ...
    fetch: async (request) => {
      ...
      if (url.hostname === "openrouter.ai") {
        providerRequests += 1;
        return jsonResponse({ error: { message: "temporary extraction outage" } }, {
          status: 503,
...
test("exact T0 URL retains extractor-503 page metadata through runner and every report projection", async () => {
  ...
  }, { availableTools: ["fetch_public_source"] })) updates.push(update);
  ...
  const reportHtml = renderToStaticMarkup(React.createElement(ReportSheet, {
