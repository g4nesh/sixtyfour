...
test("frontier batches stay on the minimum executable tier until lower tiers exhaust", () => {
  ...
  assert.ok(second.events.some((event) =>
    event.name === "source.tier_advanced" && event.payload.sourceTier === 2));
});
...
test("frontier schedules every surviving compiler variant on its legal source lane", () => {
  ...
  const empty = search.emptySearchGraph(
    "run_compiler_frontier",
    target.normalizedQuery,
    "2026-08-20T20:20:00.000Z",
  );
  const seeded = search.seedFrontier(
    empty,
    target,
  ...
  assert.ok(baseline);
  const actionNode = search.admitGraphNode(seeded.graph, {
    kind: "action",
  ...
  }, ids, "2026-08-20T20:20:00.001Z");
  const actionEdge = search.admitGraphEdge(actionNode.graph, {
  ...
  const running = search.setFrontierStatus(
    actionEdge.graph,
    [baseline.id],
    "running",
    "2026-08-20T20:20:00.001Z",
  );
  const executed = search.recordFrontierOutcome(
    running,
    baseline,
    "verified",
    "2026-08-20T20:20:00.001Z",
  );
  const candidateNode = search.admitGraphNode(executed.graph, {
    kind: "candidate",
    label: "Denise Hilary",
    status: "selected",
  ...
  }, ids, "2026-08-20T20:20:00.002Z");
  const candidateEdge = search.admitGraphEdge(candidateNode.graph, {
  ...
  const compilerEntries = candidateFrontier.graph.frontier.filter((entry) =>
  ...
  assert.ok(t2Searches.every((entry) =>
    entry.candidateId === null
    && entry.allowedTools.length === 1
    && entry.allowedTools[0] === "search_web"));
  assert.equal(candidateFrontier.value.some((entry) => entry.intent.startsWith("OSINT query ")), false);
  assert.deepEqual(
    t2Searches
      .map((entry) => entry.queryHint.match(/site:([^ ]+)/)?.[1]),
    ["github.com", "orcid.org", "scholar.google.com", "apps.apple.com"],
  );
  assert.ok(compilerEntries.some((entry) =>
    entry.sourceLaneId === "t3.institutional" && entry.queryHint.includes("site:asu.edu")));
  assert.ok(compilerEntries.some((entry) =>
    entry.sourceLaneId === "t3.institutional" && entry.queryHint.includes("filetype:pdf")));
  assert.ok(compilerEntries.some((entry) =>
    entry.sourceLaneId === "t6.general_discovery" && /-"stock photo"/.test(entry.queryHint)));
  ...
  assert.ok(search.validateSearchGraph(forged).some((issue) =>
    issue.code === "compiler_query_binding_mismatch"));
});
...
test("a zero-result name traversal reaches every canonical site, PDF, context, and name variant", () => {
  ...
  const timestamp = () => new Date(
    Date.parse("2026-08-20T20:25:00.000Z") + tick++,
  ).toISOString();
  const seeded = search.seedFrontier(
  ...
  assert.ok(compilerEntries.every((entry) =>
    ...
    && entry.allowedTools.length === 1
    && entry.allowedTools[0] === "search_web"));
  assert.equal(graph.frontier.some((entry) =>
    entry.candidateId === null && entry.allowedTools.includes("fetch_public_source")), false);
  ...
  for (let turn = 0; turn < 20; turn += 1) {
    ...
    for (const entry of selected.value) {
      ...
      graph = search.recordFrontierOutcome(
        graph,
        entry,
        "exhausted",
        timestamp(),
  ...
  for (const kind of [
    ...
    assert.ok(plan.queries.some((query) => query.kind === kind), kind);
  }
  assert.ok(executedQueries.some((query) => query.includes("filetype:pdf")));
  assert.equal(graph.frontier.every((entry) => entry.status === "exhausted"), true);
...
test("the standard runner exhausts every canonical name query before terminating", async () => {
  ...
  assert.equal(completed.report.searchGraph.frontier.every((entry) => entry.status === "exhausted"), true);
...
test("frontier schedules deterministic orthographic and initial name variants in T6", () => {
  ...
  const seeded = search.seedFrontier(
    search.emptySearchGraph(
      "run_compiler_name_variants",
      target.normalizedQuery,
      "2026-08-20T20:21:00.000Z",
    ),
    target,
    ["search_web"],
    ids,
  ...
  const t6Entries = seeded.value
    .filter((entry) => entry.sourceLaneId === "t6.general_discovery");
  assert.ok(t6Entries.some((entry) =>
    entry.intent.includes(": orthographic_name;")
    && entry.queryHint.startsWith('"Renee D angelo Smith" professional')));
  assert.ok(t6Entries.some((entry) =>
    entry.intent.includes(": initial_name;")
    && entry.queryHint.startsWith('"Renée D. Smith" professional')));
...
test("candidate-linked exact URLs open a validated T5 dependency even after the breadth cursor advanced", () => {
  const { graph: seeded, target, ids } = seededGraph(
    "Denise Hilary",
    ["search_web", "fetch_public_source", "wayback_profile_history"],
  );
  const parent = seeded.frontier.find((entry) => entry.sourceLaneId === "t1.first_party");
  assert.ok(parent);
  const action = search.admitGraphNode(seeded, {
    kind: "action",
  ...
  }, ids, "2026-08-20T19:59:59.000Z");
  const actionEdge = search.admitGraphEdge(action.graph, {
  ...
  }, ids, "2026-08-20T19:59:59.500Z");
  const runningGraph = search.setFrontierStatus(
    actionEdge.graph,
    [parent.id],
    "running",
    "2026-08-20T19:59:59.750Z",
  );
  ...
  const candidate = search.admitGraphNode(executedGraph, {
    kind: "candidate",
    label: "Denise Hilary",
    status: "selected",
    candidateId: "candidate-denise",
    data: {},
    dedupeEntityKey: "candidate:candidate-denise",
  }, ids, "2026-08-20T20:00:00.000Z");
  const candidateEdge = search.admitGraphEdge(candidate.graph, {
  ...
  }, ids, "2026-08-20T20:00:00.500Z");
  const source = search.admitGraphNode(candidateEdge.graph, {
    kind: "source",
  ...
  }, ids, "2026-08-20T20:00:01.000Z");
  const sourceEdge = search.admitGraphEdge(source.graph, {
    fromNodeId: action.value.id,
  ...
  assert.deepEqual(search.validateSearchGraph(selected.graph), [], "a selected late exact-URL dependency must remain canonical");
});
...
test("source hierarchy is tiered and denies people-search, phonebook, property, tax, family, and credential surfaces", () => {
  ...
  assert.equal(search.deterministicSourceTypeForUrl("https://scholar.google.com/citations?user=abcD_123&hl=en"), "public_document");
  assert.equal(search.sourceTierForUrl("https://scholar.google.com/citations?user=abcD_123&hl=en", "public_document"), 2);
