...
test("allowlisted temporal and footprint projections are bounded, sanitized, and export to Markdown", () => {
  ...
  assert.deepEqual(footprintEvidence.pageFootprint, {
    ...
    bounded: false,
    caveat: "Page declarations are spoofable observations from the exact fetched HTML. No referenced resource was followed, and no hosting ownership or control is inferred.",
  });
  ...
  assert.match(markdown, /Canonical URL:\*\* \[https:\/\/www\.ted\.com\/speakers\/chris\\_anderson\\_ted\]\(https:\/\/www\.ted\.com\/speakers\/chris_anderson_ted\)/);
...
test("malformed temporal windows and unbound footprint objects fail closed", () => {
  ...
  const viewModel = reportExport.createReportViewModel(report);
  const markdown = reportExport.reportViewModelToMarkdown(viewModel);
  assert.equal(viewModel.evidence.every((item) => item.pageFootprint === null), true);
  assert.equal(viewModel.evidence.every((item) => item.temporalComparison === null), true);
  assert.doesNotMatch(markdown, /Page-declared footprint|Temporal comparison|Public page metadata retained from the already-fetched profile/);
});
...
test("body-only archive changes remain explicit and malformed change dimensions fail closed", () => {
  ...
  assert.deepEqual(temporal, {
    ...
    unchangedFragmentCount: 1,
    comparisonBounded: false,
    caveat: "Archive observations bind changes in retrieved response bytes and bounded static-HTML projections to this interval; they do not identify the editor or prove archive completeness, and they do not describe browser-rendered state.",
  });
  const markdown = reportExport.reportViewModelToMarkdown(viewModel);
  assert.match(markdown, /Archived response body bytes changed:\*\* Yes/);
...
test("React-PDF smoke produces application/pdf bytes", { skip: process.env.ATLAS_PDF_SMOKE !== "1" }, async () => {
  ...
  const viewModel = reportExport.createReportViewModel(report);
  assert.equal(viewModel.evidence.some((item) => item.temporalComparison !== null), true);
  const temporal = viewModel.evidence.find((item) => item.temporalComparison)?.temporalComparison;
  ...
  assert.equal(viewModel.evidence.some((item) => item.pageFootprint?.footprintHash === footprintProjectionHash), true);
  assert.equal(viewModel.evidence.some((item) => item.contentLabel === "Passive page metadata observation"), true);
