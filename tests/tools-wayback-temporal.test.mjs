...
test("Wayback compares exact raw bodies plus bounded static-HTML text, metadata, and structure", async () => {
  ...
  const requested = [];
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    clock: () => Date.parse("2026-08-20T18:00:00Z"),
    fetch: async (input, init) => {
      const url = new URL(String(input));
      requested.push(url.href);
      assert.equal(init?.method, "GET");
      assert.notEqual(url.pathname.includes("save"), true, "the adapter is read-only and never invokes Save Page Now");
      ...
      return new Response(`<!doctype html><html lang="en-us"><head>
...
test("Wayback chooses newest plus an earlier distinct digest after an edit-and-revert sequence", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      return new Response(timestamp === "20210101000000"
  ...
  assert.deepEqual(result.data.captureTimeline.map((item) => item.digest), ["DIGEST-A", "DIGEST-B", "DIGEST-A"]);
...
test("Wayback never reinterprets markup-looking active-tag attributes", async () => {
  ...
  assert.deepEqual(result.data.temporalChange.addedTextFragments, []);
  assert.deepEqual(result.data.temporalChange.removedTextFragments, []);
  assert.equal(result.data.snapshots.every((snapshot) =>
    snapshot.metadata.description === "REAL-DESCRIPTION"), true);
  const serialized = JSON.stringify(result);
...
test("Wayback removes nested inactive containers from text, metadata, structure, and diffs", async () => {
  ...
  assert.deepEqual(result.data.temporalChange.addedTextFragments, []);
  assert.deepEqual(result.data.temporalChange.removedTextFragments, []);
  assert.equal(result.data.snapshots.every((snapshot) => snapshot.metadata.description === null), true);
  const serialized = JSON.stringify(result);
...
test("Wayback decodes references and format controls before retaining metadata, excerpts, or diffs", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      return new Response(`<html><head><title>${token}</title><meta name="description" content="${email}"></head><body><p>Stable public biography.</p><p>${token}</p><p>${zeroWidthToken}</p></body></html>`, {
        headers: {
  ...
  assert.equal(result.data.snapshots.every((snapshot) => snapshot.metadata.title === null), true);
  assert.equal(result.data.snapshots.every((snapshot) => snapshot.metadata.description === null), true);
  assert.equal(result.data.snapshots.every((snapshot) => snapshot.responseContentType === "text/html; charset=utf-8"), true);
  assert.deepEqual(result.data.temporalChange.addedTextFragments, []);
  assert.deepEqual(result.data.temporalChange.removedTextFragments, []);
  const serialized = JSON.stringify(result.data);
  for (const forbidden of ["ghp_", "ghp&#", "commat", "private@example.com", "other@example.com", "x-padding", "x-token"]) {
...
test("Wayback retains common named references with case-correct text", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      if (url.pathname === "/cdx/search/cdx") return jsonResponse([
        header,
        ["20240101000000", targetUrl, "text/html", "200", "DIGEST-COMMON", "180"],
      ]);
      return new Response('<html><head><title>&Eacute;lodie&rsquo;s R&eacute;sum&eacute; &mdash; Research &copy;</title><meta name="description" content="&ldquo;Registered&rdquo; &reg; profile"></head><body><p>Public R&eacute;sum&eacute;.</p></body></html>', {
        headers: { "content-type": "text/html; charset=utf-8" },
      });
    },
  });
...
test("Wayback requires exact ASCII closing-tag delimiters for every inactive container", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      return new Response(`<html><head><title>Stable profile</title></head><body><p>Stable public biography.</p><script>${hidden}</script!><p>${hidden}</p></script><textarea>${hidden}</ textarea><p>${hidden}</p></textarea><template>${hidden}</template\u00a0><p>${hidden}</p></template></body></html>`, {
        headers: { "content-type": "text/html" },
      });
    },
  });
...
test("Wayback hashes but never retains credential literals from archived HTML", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      if (url.pathname === "/cdx/search/cdx") return jsonResponse([
        header,
        ["20240101000000", targetUrl, "text/html", "200", "DIGEST-SECRET", "400"],
      ]);
      return new Response(`<html><head><title>${credentials[0]}</title><meta name="description" content="${credentials[1]}"></head><body>${credentials.slice(2).map((credential) => `<p>${credential}</p>`).join("")}</body></html>`, {
        headers: { "content-type": "text/html" },
      });
    },
  });
...
test("Wayback checks complete normalized text before excerpt bounding", async () => {
  ...
  const result = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      ...
      if (url.pathname === "/cdx/search/cdx") return jsonResponse([
        header,
        ["20240101000000", targetUrl, "text/html", "200", "DIGEST-BOUNDARY", "180"],
      ]);
...
test("Wayback rejects secret-bearing URLs and reversed ranges without any request", async () => {
  let calls = 0;
  const context = { fetch: async () => { calls += 1; throw new Error("must not fetch"); } };
...
test("Wayback treats absent or redirected exact captures as non-proof and never substitutes another snapshot", async () => {
  const targetUrl = "https://profile.example/about";
  let calls = 0;
  const absent = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async () => {
      calls += 1;
      return jsonResponse([header, ["20200101000000", "https://foreign.example/about", "text/html", "200", "FOREIGN", "10"]]);
    },
  });
  assert.equal(absent.status, "not_found");
  assert.equal(absent.data.snapshots.length, 0);
  assert.match(absent.diagnostics.find((item) => item.code === "wayback_captures_not_observed").message, /does not prove/);

  const requested = [];
  const redirected = await inspectWaybackHistory({ url: targetUrl, candidate }, {
    fetch: async (input) => {
      calls += 1;
      const url = new URL(String(input));
      requested.push(url.href);
