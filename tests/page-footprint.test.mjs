...
test("page footprint projects only bounded page-declared metadata and resource host families", () => {
  ...
  assert.deepEqual(footprint, {
    ...
    bounded: false,
    spoofable: true,
    scopeNote: "Page declarations are inert, bounded, and spoofable. They show only what this exact fetched HTML referenced at observation time; no listed resource was fetched, and ownership, hosting, control, deployment, authorship, and completeness are not inferred.",
  });
...
test("page footprint drops contacts, addresses, secrets, prompt injection, and unsafe resource URLs", () => {
  ...
  const footprint = extractPublicPageFootprint({ html, finalUrl: "https://profile.example.com/token/super-secret-value" });
  ...
  const serialized = JSON.stringify(footprint);
  for (const forbidden of ["602", "Main Street", "person@example.com", "never-retain-me", "super-secret-value", "system prompt"]) {
...
test("page footprint ignores declarations forged inside inactive markup and comments", () => {
  ...
  const serialized = JSON.stringify(footprint);
  for (const forged of ["Comment Fake", "Style Fake", "Template Fake", "Script Fake", "TemplateForgery", "forged", "script.cloudflare.net", "template.cloudfront.net"]) {
  ...
  for (const tag of ["script", "textarea", "template"]) {
    for (const invalidClose of [`</${tag}!>`, `</ ${tag}>`, `</${tag}\u00a0>`]) {
      ...
      assert.deepEqual(strictClose.declaredApplications.generators, ["Real Generator"], `${tag} ${JSON.stringify(invalidClose)}`);
...
test("page footprint checks complete fields before bounding them", () => {
  ...
  const hostnameCredential = extractPublicPageFootprint({
    finalUrl: "https://profile.example.com/person",
    html: '<img src="https://ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.evil.example/x">',
  });
