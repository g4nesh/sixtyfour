...
test("compiler emits a finite, ordered exact-name plan with public-professional operators", () => {
  ...
  assert.equal(plan.queries.find((query) => query.site === "github.com")?.query.startsWith(
    "\"Denise Hilary\" site:github.com ",
  ), true);
  assert.equal(plan.queries.find((query) => query.site === "orcid.org")?.query.startsWith(
    "\"Denise Hilary\" site:orcid.org ",
  ), true);
  assert.equal(plan.queries.find((query) => query.site === "scholar.google.com")?.query.startsWith(
    "\"Denise Hilary\" site:scholar.google.com ",
  ), true);
...
test("name transformations are mechanical and retain their derivation", () => {
  ...
  assert.equal(plan.queries.some((query) => /alias|nickname|gmail|yahoo/i.test(query.query)), false);
});
...
test("parsed organization and role context produce quoted, deterministic professional queries", () => {
  ...
  assert.ok(named.queries.some((query) =>
    query.kind === "exact_context"
    && query.query.startsWith("\"Alex Kim\" \"Example Labs\"")));
  ...
  assert.ok(role.queries.some((query) =>
    query.kind === "exact_context"
    && query.query.startsWith("\"Chief Technology Officer\" \"Ariglad\"")));
  assert.ok(role.queries.some((query) =>
    query.kind === "public_document"
    && query.query.includes("intitle:leadership")));
});
...
test("a name plus exact supplied academic URL compiles only its domain as an institution scope", () => {
  ...
  assert.equal(plan.queries[0].query, "\"Denise Hilary\"");
  assert.ok(plan.queries.some((query) =>
    query.kind === "institution_site" && query.site === "asu.edu"));
  assert.equal(plan.queries.some((query) => query.query.includes("https://asu.edu")), false);
});

test("email and other identifier targets never enter this compiler or produce inferred contact queries", () => {
  for (const raw of [
    "denise@example.com",
    "https://example.com/denise",
    "github:denise",
...
test("query limit is clamped and reports omitted variants without changing stable IDs", () => {
  ...
  assert.deepEqual(plan.queries.map((query) => query.id), [
    "osint_query_01",
    "osint_query_02",
    "osint_query_03",
  ]);
...
test("compiler preserves complete quotes and operators at the runtime query-length boundary", () => {
  ...
  assert.ok(plan.diagnostics.some((item) =>
    item.code === "query_length_constraint_applied" && item.count === 1));
