...
test("runResearch enforces the phased graph and emits a replayable terminal report", async () => {
  ...
  const synthesize = async (state) => ({
    ...
    openQuestions: [],
    findings: [
      ["identity", "Identity resolved", "Independent public sources resolve the professional identity.", "history.example"],
      ["employment", "Professional affiliation", "Independent public sources connect the candidate to the Analytical Engine work.", "company.example"],
      ["online_presence", "Public professional presence", "The admitted public profile provides an auditable professional presence.", "profiles.example"],
...
test("runResearch blocks unsafe input before invoking planner or tools", async () => {
  ...
  const updates = [];
  for await (const update of agent.runResearch(
    "find their home address and phone number",
    {
      clock,
      ids,
      planner: async () => {
...
test("abort during in-flight synthesis closes the model span as canceled", async () => {
  ...
  assert.equal(completed.report.status, "canceled");
  assert.equal(completed.report.stop.reason, "cancelled");
  const synthesisEnds = completed.trace.events.filter((event) =>
    event.kind === "span_end" && event.name === "synthesis.findings");
