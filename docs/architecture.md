...
Deep named-person runs are subject-focused rather than candidate-breadth searches. The first unique neutral query-subject anchor may own candidate-bound work; alternate candidates remain in the graph for audit but their queued branches are exhausted. If more than one structurally eligible anchor exists, all candidate-bound work fails closed while candidate-free canonical searches may continue. This anchor is a routing control only: it changes neither identity score nor evidence trust, is not a merge key, and cannot resolve a same-name ambiguity. One prioritized exact-subject page may be fetched after the initial T1 grounding batch; the remaining exact source-family queries and candidate-bound leads then interleave by the normal tier/cost policy instead of stopping at that first useful page.

`lib/search/osint-query-compiler.ts` compiles at most 16 finite public-professional query variants for one exact parsed subject. Human-name comparison preserves Unicode letters, marks, and numbers while generic tool identifiers keep a separate ASCII-only slug grammar. A named-person target may include bounded exact role, organization/company, coarse city/region, and adult education context; no phrase is truncated into malformed operator syntax. The neutral quoted baseline is always first, and negative noise exclusions exist only on visibly labeled refinements. The compiler deliberately spends its remaining capacity on exact LinkedIn post/activity, X/Twitter status-thread, Reddit discussion/AMA, authored-content, interview/podcast, scholarly, institutional, metadata, regulatory, and document branches rather than inventing orthographic or initial-based aliases. Source lanes select their canonical query from that program, and `execute_expansion` replaces any model-suggested query with the frontier-owned string before transport. `site:` scopes are closed to GitHub, LinkedIn, X/Twitter, Reddit, ORCID, Google Scholar, OpenReview, Semantic Scholar, OpenAlex, official App Store listings, and at most two caller-admitted academic domains. Social query annotations pass a second exact-permalink gate: profiles, home feeds, jobs, search, community roots, and lookalike hosts cannot become fetch capabilities. Every operator result remains transport metadata until direct fetch admission.
...
- `lib/providers`: provider-neutral hardened client contracts plus the externally configured OpenRouter client; direct-provider adapters remain internal test surfaces and are not selectable through HTTP, CLI, Compose, or deployment variables.
...
Replay APIs remain credential-free and zero-network for deterministic tests and audit, but the browser workbench does not expose replay or depth selection and always requests live Deep execution. API category filters accept only the exact report-schema enum values and reject empty, unknown, or non-string entries instead of silently narrowing the requested coverage. Non-local live HTTP ingress requires explicit enablement, a server-side OpenRouter credential, and a 32-byte-or-longer Atlas access token. The dashboard exchanges that independent token once for a signed 30-minute `__Host-` HttpOnly, Secure, SameSite session; the raw token is not retained in browser storage or attached to research requests. Bearer authentication remains available for CLI and machine clients. The only unauthenticated bypass is an explicit development binding on `localhost`, `127.0.0.1`, or `[::1]`. `/api/health` reports only whether live mode is usable and whether browser authorization is required; it never exposes the provider, model, or secret material. The Node CLI uses the loopback path directly. `worker/index.ts` applies browser security headers to API, image, and Vinext responses.

## Live model protocol

Live mode calls OpenRouter from LangGraph's `plan_expansion` and `synthesize` nodes. `LIVE_PROVIDER` may be unset or pinned to `openrouter`; any other nonempty value fails closed, and direct OpenAI, Gemini, Anthropic, or secondary-search credentials are not accepted by HTTP, CLI, Compose, or example environments. Structured turns use custom function schemas, automatic tool choice, `parallel_tool_calls: false`, and one expected structured submission per provider turn. Discovery uses the current OpenRouter `openrouter:web_search` server tool with `max_uses: 1` for each Atlas search action and normalizes only server-attested HTTPS citation annotations into discovery capabilities. The planner receives a compact view of the selected frontier and must bind each action to one selected ID. The deterministic runner can execute an approved same-tier batch concurrently, up to four actions. Every provider attempt is reserved before dispatch and settled independently, so repair and concurrent extraction calls count against LLM, network, token, and cost budgets. OpenRouter `reasoning_details`, when returned, are opaque continuation data and are passed back unchanged on the next provider turn. They are never logged or streamed. Only provider-reported prompt, completion, reasoning, and cached-input token counts are exposed; unavailable values remain `null` with a reason.
...
One pure JSON-safe `ReportViewModel` feeds the browser, deterministic Markdown, and PDF presentation. Its opening is a narrative-first public-professional briefing assembled only from already-admitted, same-candidate findings, exact candidate-bound observations, and their cited HTTPS sources; it never invokes a second prose model or borrows a fact from another identity branch. Policy scores, thresholds, search coverage, and stop mechanics remain available in a subordinate technical audit and are labeled as rule-based decision metadata rather than calibrated probabilities. The view retains the complete candidate-branch count, profiles up to the five highest-ranked candidates, and labels every finding and evidence reference with its candidate scope. Evidence receives stable `E01...` references; exact fetched excerpts and canonical structured API claims remain visibly distinct. Markdown escapes hostile syntax and omits raw provider/tool payloads. The browser dynamically imports React-PDF only when the operator clicks PDF download; the Worker API always returns the canonical JSON/NDJSON report, never server-rendered PDF bytes. React Flow and ELK are likewise confined to the client graph chunk, and the accessible list view remains usable without canvas layout.
