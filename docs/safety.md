...
The checked-in deployment is replay-only. Non-local live HTTP research fails closed unless an operator sets `ATLAS_LIVE_ENABLED=true`, configures a server-side provider key, configures an `ATLAS_API_TOKEN` of at least 32 bytes, and supplies the matching bearer token. The development script's unauthenticated bypass is loopback-only and must never be enabled on public ingress. Public deployments should still add Cloudflare Access or an equivalent identity-aware gateway plus per-principal request, token, and cost limits. None of these server bindings is exposed to the browser.
...
The frontier is seeded only with safe public-professional pivots: names, organizations, roles, exact public HTTPS URLs/domains/handles, repositories, DOI/ORCID/publications/packages, and exact user-supplied emails. Raw phone/address input is recognized only to refuse; it never becomes a graph node. Search proceeds T0 exact supplied identifiers → T1 first-party pages → T2 structured professional records and official organization filings → T3 institutional/publisher sources → T4 reputable media → T5 candidate-linked Wayback → T6 general discovery. Higher tiers cannot run while legal lower-tier work remains. Search ordering changes neither evidence admission nor confidence.
...
Direct fetching is HTTPS-only and candidate-scoped. A provider-attested URL is stored behind an opaque, run-local lead ID; the gate resolves that exact hidden URL for the matching `candidateId`, so a model-rewritten or tracking-stripped lookalike is not sufficient authorization. Credential/private-data query keys, fragments, nested redirect values, URL userinfo, and decoded contact data are rejected before egress or durable evidence admission. Redirects are followed manually only after the new destination passes the same checks. Localhost, loopback, private, link-local, multicast, metadata, reserved/test-network IP literals, suspicious ports, URL credentials, and DNS answers that include blocked address space are rejected. MIME type, byte size, duration, redirects, and retries are capped. General arbitrary-host fetching requires an injected trusted hostname resolver or controlled egress proxy and otherwise fails closed; provider search annotations remain discovery-only.
