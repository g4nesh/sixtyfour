...
export function canonicalGraph(value: unknown): CanonicalSearchGraph | null {
  ...
  if (
    ...
    graph.edges.some(
      (edge) => !isRecord(edge) || edge.schemaVersion !== 2 || !edge.id || !edge.fromNodeId || !edge.toNodeId,
    ) ||
    graph.frontier.some(
      (entry) =>
        !isRecord(entry) ||
        entry.schemaVersion !== 2 ||
        entry.id !== entry.frontierEntryId ||
        entry.id !== entry.actionId,
    ) ||
...
function retainsIds(previous: readonly { id: string }[], next: readonly { id: string }[]): boolean {
 ...
 */
export function mergeGraphSnapshot(current: CanonicalSearchGraph | null, value: unknown): CanonicalSearchGraph | null {
...
export function deterministicPositions(graph: CanonicalSearchGraph): Map<string, GraphPoint> {
  ...
  const nodeOriginY = GRAPH_LAYOUT_PADDING + graph.edges.length * GRAPH_FALLBACK_EDGE_LANE_GAP + GRAPH_NODE_GAP;
...
function segmentIntersectsRectangle(start: GraphPoint, end: GraphPoint, rectangle: GraphRectangle): boolean {
  const epsilon = 0.001;
  if (Math.abs(start.x - end.x) <= epsilon) {
    const minimumY = Math.min(start.y, end.y);
    const maximumY = Math.max(start.y, end.y);
    return (
      start.x > rectangle.left && start.x < rectangle.right && maximumY > rectangle.top && minimumY < rectangle.bottom
    );
  }
  if (Math.abs(start.y - end.y) <= epsilon) {
    ...
    return (
      start.y > rectangle.top && start.y < rectangle.bottom && maximumX > rectangle.left && minimumX < rectangle.right
    );
  }
  return true;
}
...
function pointOnRectangleBoundary(point: GraphPoint, rectangle: GraphRectangle): boolean {
  ...
  return (
    (withinVertical &&
      (Math.abs(point.x - rectangle.left) <= epsilon || Math.abs(point.x - rectangle.right) <= epsilon)) ||
    (withinHorizontal &&
      (Math.abs(point.y - rectangle.top) <= epsilon || Math.abs(point.y - rectangle.bottom) <= epsilon))
  );
}
...
function properOrthogonalSegmentCrossing(
  ...
  const epsilon = 0.001;
  const leftVertical = Math.abs(leftStart.x - leftEnd.x) <= epsilon && Math.abs(leftStart.y - leftEnd.y) > epsilon;
  const leftHorizontal = Math.abs(leftStart.y - leftEnd.y) <= epsilon && Math.abs(leftStart.x - leftEnd.x) > epsilon;
  const rightVertical = Math.abs(rightStart.x - rightEnd.x) <= epsilon && Math.abs(rightStart.y - rightEnd.y) > epsilon;
  const rightHorizontal =
    Math.abs(rightStart.y - rightEnd.y) <= epsilon && Math.abs(rightStart.x - rightEnd.x) > epsilon;
 ...
 */
export function unrelatedEdgeCrossings(graph: CanonicalSearchGraph, layout: GraphLayout): GraphEdgeCrossing[] {
 ...
 */
export function isCollisionFreeGraphLayout(graph: CanonicalSearchGraph, layout: GraphLayout): boolean {
  ...
  for (let leftIndex = 0; leftIndex < graph.nodes.length; leftIndex += 1) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < graph.nodes.length; rightIndex += 1) {
      ...
      if (!rightPoint || rectanglesIntersect(leftRectangle, nodeRectangle(rightPoint, GRAPH_NODE_GAP / 2)))
        return false;
    }
  }
