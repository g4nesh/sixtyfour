...
export function SourceLadder({ graph, collapsed, onToggle }: {
  ...
  return <aside className={`source-ladder ${collapsed ? "is-collapsed" : ""}`} aria-label="Website source search hierarchy">
    ...
    <ol id="source-ladder-steps" hidden={collapsed}>
      {rows.map(({ item, count, active, complete, rejected }) => <li key={item.id} className={active ? "is-active" : complete ? "is-complete" : rejected ? "is-rejected" : ""}>
        ...
        <span className="ladder-count" aria-label={`${count} graph nodes`}>{active ? "live" : count}</span>
      </li>)}
    </ol>
