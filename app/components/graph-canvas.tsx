...
function GraphCanvasInner({ graph, selectedNodeId, onSelectNode, focusedStableId, fitRequest }: {
  ...
  const preparedNodes = useMemo(() => flowNodes(graph, activeLayout, selectedNodeId, focusedStableId, onSelect), [activeLayout, focusedStableId, graph, onSelect, selectedNodeId]);
  const preparedEdges = useMemo(() => flowEdges(graph, activeLayout, focusedStableId), [activeLayout, focusedStableId, graph]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void elkGraphLayout(snapshot).then((nextLayout) => {
        if (
          nextLayout
