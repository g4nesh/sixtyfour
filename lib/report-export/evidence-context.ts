...
const PAGE_CANONICAL_STATUSES = new Set<ReportPageCanonicalStatus>(["not_declared", "accepted_same_page", "discarded"]);
...
function safeCanonicalUrl(value: JsonValue | undefined, state: ProjectionState): string | null {
  if (typeof value !== "string") return null;
  ...
  if (!safe || containsRestrictedPublicContent(safe) || urlContainsRestrictedParameters(safe)) return null;
  try {
    const url = new URL(safe);
    if (url.port !== "" || url.hash !== "" || !safePublicHostname(url.hostname)) return null;
    return url.href;
  } catch {
    return null;
  }
...
function safeCanonicalStatus(value: JsonValue | undefined): ReportPageCanonicalStatus | null {
  return typeof value === "string" && PAGE_CANONICAL_STATUSES.has(value as ReportPageCanonicalStatus)
...
function safeLanguage(value: JsonValue | undefined, state: ProjectionState): string | null {
  ...
  return /^[a-z]{2,3}(?:-[a-z0-9]{2,8}){0,2}$/i.test(language) ? language.toLocaleLowerCase("en-US") : null;
}

export function projectTemporalComparison(canonicalSubset: unknown): ReportTemporalComparisonView | null {
  ...
  if (!observedAfter || !observedOnOrBefore || Date.parse(observedAfter) > Date.parse(observedOnOrBefore)) return null;
...
export function projectPageFootprint(canonicalSubset: unknown): ReportPageFootprintView | null {
  const canonical = objectValue(canonicalSubset);
  const source = objectValue(canonical?.pageFootprint);
  const footprintHash =
    typeof canonical?.pageFootprintHash === "string" && /^sha256:[a-f0-9]{64}$/.test(canonical.pageFootprintHash)
  ...
  const acceptedCanonicalUrl =
    declaredCanonicalStatus === "accepted_same_page" ? safeCanonicalUrl(source.canonicalUrl, state) : null;
  const canonicalStatus =
    declaredCanonicalStatus === "accepted_same_page" && !acceptedCanonicalUrl ? null : declaredCanonicalStatus;
  ...
  const observedProviderFamilies = safeStringList(source.observedProviderFamilies, {
    ...
    state,
    accept: (item): item is ReportPageProviderFamily => PAGE_PROVIDER_FAMILIES.has(item as ReportPageProviderFamily),
