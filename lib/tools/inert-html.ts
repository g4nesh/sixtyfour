...
const RAW_TEXT_CONTAINERS = new Set(["iframe", "plaintext", "script", "style", "textarea", "title", "xmp"]);
...
function tagNameDelimiter(character: string | undefined): boolean {
  return character === undefined || character === "/" || character === ">" || ASCII_TAG_WHITESPACE.has(character);
}
...
export function decodeHtmlTextForPolicy(value: string): string | null {
  ...
  return [...decoded]
    .filter((character) => {
      ...
      const disallowedAscii =
        (codePoint < 32 && ![9, 10, 13].includes(codePoint)) || (codePoint >= 127 && codePoint <= 159);
      const directionalControl =
        (codePoint >= 0x202a && codePoint <= 0x202e) || (codePoint >= 0x2066 && codePoint <= 0x2069);
      const zeroWidthFormatControl =
        codePoint === 0x200b ||
        codePoint === 0x200c ||
        codePoint === 0x200d ||
        codePoint === 0x2060 ||
...
function consumeInactiveContainer(html: string, opening: HtmlTagToken): ConsumedContainer {
...
function scriptIsJsonLd(attributes: string): boolean {
  ...
  return (type?.[1] ?? type?.[2] ?? type?.[3] ?? "").trim().toLocaleLowerCase("en-US") === "application/ld+json";
}
...
function projectedTagSource(source: string): string {
  ...
  const body = source.slice(1, -1).replaceAll("<", "&lt;").replaceAll(">", "&gt;");
  return `<${body}>`;
}
...
export function projectInertHtml(html: string): InertHtmlProjection {
  ...
  while (cursor < html.length) {
    ...
    const closingTag = consumed.closingTag ? projectedTagSource(consumed.closingTag) : "";
