...
export async function searchGithubPublicUsersByExactName(
  ...
  try {
    ...
    return finish(startedAt, now, hardened?.code === "budget_exhausted" ? "skipped" : "failed", null, [{
      code: hardened?.code ?? "github_public_user_unavailable",
      severity: hardened?.code === "budget_exhausted" ? "info" : "warning",
      message: hardened?.code === "budget_exhausted"
    ...
    }], hardened?.requests ?? requests, bytesRead, true);
  }
  ...
  for (const item of searchItems) {
    ...
    try {
      ...
      if (hardened?.code === "budget_exhausted") {
        return finish(startedAt, now, "partial", {
          exactName,
          matches,
  ...
  if (excludedRows > 0) diagnostics.push({
  ...
  if (matches.length === 0) diagnostics.push({
    code: "github_exact_name_not_observed",
    severity: "info",
    message: "No exact public-name match was observed in the bounded GitHub user records; this is not evidence that no profile exists.",
    retryable: detailIncomplete || incompleteResults || (totalCountReported ?? 0) > boundedResultCount,
  });
  if (detailIncomplete) diagnostics.push({
  ...
  const incomplete = detailIncomplete
    || incompleteResults
    || (totalCountReported ?? boundedResultCount) > boundedResultCount;
