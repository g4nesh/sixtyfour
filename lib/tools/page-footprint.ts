...
export type PageCanonicalStatus = "not_declared" | "accepted_same_page" | "discarded";
...
const PROMPT_INJECTION_PATTERN =
  /\b(?:ignore|disregard|override)\b.{0,32}\b(?:previous|prior|system|developer|instructions?|prompt)\b|\b(?:system|developer|assistant)\s*(?:message|prompt)\s*:|\byou\s+are\s+(?:chatgpt|an?\s+assistant)\b/i;
const SECRET_LITERAL_PATTERN =
  /\b(?:bearer\s+[a-z0-9._~+/=-]{8,}|(?:api|access|auth|session)[ _-]?(?:key|token|secret)\s*[:=]|(?:password|passwd|private[ _-]?key)\s*[:=]|sk-[a-z0-9_-]{12,})/i;
const URL_WITH_QUERY_PATTERN = /https?:\/\/[^\s<>"']+\?[^\s<>"']+/i;
const SENSITIVE_PATH_SEGMENT_PATTERN =
  /\/(?:access[-_]?token|api[-_]?key|auth|authorization|bearer|credential|email|home[-_]?address|jwt|pass(?:code|word)|phone|private[-_]?key|secret|session|token)(?:\/|$)/i;
...
const PROVIDER_RULES: ReadonlyArray<{
  ...
  {
    suffixes: ["googleapis.com", "gstatic.com", "googleusercontent.com", "firebaseapp.com"],
    family: "google-hosted-assets",
  },
...
function safeText(rawValue: string | null | undefined, maximumCharacters: number, state: AnalysisState): string | null {
...
function declaredBaseUrl(html: string, finalUrl: URL, state: AnalysisState): URL | null {
  ...
  return declarations.length === 0 ? finalUrl : normalizePublicHttpsUrl(declarations[0], finalUrl);
}
...
function collectCanonical(
  ...
  const samePage = candidates.every(
    (candidate) =>
  ...
  const uniquePages = new Set(
    candidates.map((candidate) => `${candidate.origin}${candidate.pathname}?${canonicalQueryFingerprint(candidate)}`),
  );
...
function addBounded(target: Set<string>, value: string, maximum: number, state: AnalysisState): void {
...
function collectTypesFromJsonLd(root: unknown, types: Set<string>, maximum: number, state: AnalysisState): void {
...
function collectJsonLdTypes(html: string, limits: AnalysisLimits, state: AnalysisState): string[] {
