...
function sanitizePersonName(value: string | undefined): string | null {
  ...
  return words.every((word) => /^[\p{L}\p{M}][\p{L}\p{M}'’.-]{0,63}$/u.test(word))
    ? normalized
    : null;
}
...
function sanitizeProfessionalPhrase(value: string | undefined): string | null {
  ...
  if (
    /\b(?:api[-_ ]?keys?|access[-_ ]?tokens?|passwords?|credentials?|private[-_ ]?keys?|secret(?:s|[-_ ]?keys?)?)\b/i.test(normalized)
    || /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,63}\b/i.test(normalized)
    || /(?:^|\s)\+?\d[\d(). -]{7,}\d(?:\s|$)/.test(normalized)
    || /\b\d{1,8}\s+[\p{L}\p{M} .'-]+\s(?:street|st|avenue|ave|road|rd|boulevard|blvd|lane|ln|drive|dr)\b/iu.test(normalized)
  ) return null;
  const words = normalized.split(" ");
...
function academicDomain(value: string): string | null {
  ...
  if (
    labels.length < 2
    || labels.some((label) => label.length > 63 || label.startsWith("-") || label.endsWith("-"))
  ) return null;
  const isAcademic = normalized.endsWith(".edu")
    || /\.(?:ac|edu)\.[a-z]{2}$/.test(normalized);
  return isAcademic ? normalized : null;
}

function institutionScopes(
...
function orthographicNameVariant(name: string): string | null {
  const folded = normalizeWhitespace(name
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[’‘`]/g, "'")
    .replace(/['.-]+/g, " "));
...
function boundedContextBody(subject: string, phrases: readonly string[]): {
  body: string | null;
  omitted: number;
} {
  const parts = [subject];
 ...
 */
export function compileOsintQueries(
  target: ParsedTarget,
  options: CompileOsintQueryOptions = {},
): OsintQueryPlan {
  ...
  if (institutions.rejectedCount > 0) {
    diagnostics.push({
      ...
      message: "One or more institution domains were invalid, non-academic, duplicate beyond the bounded scope, or over the limit.",
  ...
  const deduped = drafts.filter((draft, index, all) =>
    all.findIndex((candidate) => candidate.body === draft.body) === index);
